...
{
    static std::shared_ptr<GroupService> instance(nullptr);
    if (!instance) {
        throw std::runtime_error("GroupService not initialized. Use constructor to create an instance first.");
    }
    return instance;
}

GroupService::GroupService(DB& db) : db_(db)
{
}
